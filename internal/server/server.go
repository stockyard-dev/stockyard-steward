...
import (
	...
	"github.com/stockyard-dev/stockyard-steward/internal/store"
)
...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("steward: license persist failed: %v", err)
	...
	s.limits = ProLimits()
	s.limMu.Unlock()
	log.Printf("steward: license activated via dashboard, persisted to %s/%s", s.dataDir, licenseFilename)
...
func (s *Server) tierInfo(w http.ResponseWriter, r *http.Request) {
	...
	if tier == "trial-required" {
		...
		resp["message"] = "Your trial is not active. Reads work, but you cannot add or change expenses until you start a 14-day trial or activate an existing license key."
	} else {
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("steward: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("steward: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"expenses": oe(s.db.Search(q, filters))})
		return
	}
...
// update accepts a full or partial expense. Empty fields preserve the
// existing values, except amount=0 is treated as "set to zero" only when
// it differs from existing — otherwise preserved.
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	// Amount=0 is ambiguous (could mean unset or zero). Preserve existing
	// when zero comes in — users editing other fields shouldn't accidentally
	// zero out their amounts. To explicitly zero, use the API directly.
	if patch.Amount == 0 {
