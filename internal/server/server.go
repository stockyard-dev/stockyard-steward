package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-steward/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-steward"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/expenses",s.handleList)
    s.mux.HandleFunc("POST /api/expenses",s.handleCreate)
    s.mux.HandleFunc("POST /api/expenses/{id}/approve",s.handleApprove)
    s.mux.HandleFunc("POST /api/expenses/{id}/reject",s.handleReject)
    s.mux.HandleFunc("DELETE /api/expenses/{id}",s.handleDelete)
