...
<div class="hdr"><h1><span>&#9670;</span> STEWARD</h1><button class="btn btn-p" onclick="openForm()">+ Add</button></div>
...
async function load(){var r=await fetch(A+'/expenses').then(function(r){return r.json()});items=r.expenses||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.description||'').toLowerCase().includes(q)||(i.amount||'').toLowerCase().includes(q)||(i.category||'').toLowerCase().includes(q)||(i.vendor||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.description)+'</div>';
if(i.amount)h+='<div class="item-sub">'+esc(i.amount)+'</div>';
...
if(i.vendor)h+='<span>'+esc(i.vendor)+'</span>';
if(i.date)h+='<span>'+esc(i.date)+'</span>';
if(i.payment_method)h+='<span>'+esc(i.payment_method)+'</span>';
if(i.receipt)h+='<span>'+esc(i.receipt)+'</span>';
...
function formHTML(item){var i=item||{description:"",amount:"",category:"",vendor:"",date:"",payment_method:"",receipt:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' STEWARD</h2>';
h+='<div class="fr"><label>Description</label><textarea id="f-description" rows="3">'+esc(i.description)+'</textarea></div>';
h+='<div class="fr"><label>Amount</label><input id="f-amount" value="'+esc(i.amount)+'"></div>';
h+='<div class="fr"><label>Category</label><input id="f-category" value="'+esc(i.category)+'"></div>';
h+='<div class="fr"><label>Vendor</label><input id="f-vendor" value="'+esc(i.vendor)+'"></div>';
h+='<div class="fr"><label>Date</label><input id="f-date" type="date" value="'+esc(i.date)+'"></div>';
h+='<div class="fr"><label>Payment Method</label><input id="f-payment-method" value="'+esc(i.payment_method)+'"></div>';
h+='<div class="fr"><label>Receipt</label><input id="f-receipt" value="'+esc(i.receipt)+'"></div>';
...
async function submit(){var body={description:document.getElementById('f-description').value.trim(),amount:document.getElementById('f-amount').value.trim(),category:document.getElementById('f-category').value.trim(),vendor:document.getElementById('f-vendor').value.trim(),date:document.getElementById('f-date').value.trim(),payment_method:document.getElementById('f-payment-method').value.trim(),receipt:document.getElementById('f-receipt').value.trim(),status:document.getElementById('f-status').value.trim()};
